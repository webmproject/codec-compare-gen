...
StatusOr<WP2::Data> EncodeOpenjpeg(const TaskInput& input,
  ...
  parameters.numresolution = 1;  // Otherwise encoding fails for tiny images.
  ...
  if (input.codec_settings.quality == kQualityLossless) {
    ...
    parameters.tcp_rates[0] = 1.f;
  } else {
    // TODO(yguyon): Implement 4:2:0.
    CHECK_OR_RETURN(
        input.codec_settings.chroma_subsampling == Subsampling::k444, quiet);
    parameters.subsampling_dx = parameters.subsampling_dy = 1;
    parameters.tcp_rates[0] = 100.f / input.codec_settings.quality;
...
StatusOr<std::pair<Image, double>> DecodeOpenjpeg(
  ...
  for (uint32_t i = 0; i < opj_image->numcomps; ++i) {
    ...
    bpc = comp.prec;
    CHECK_OR_RETURN(comp.prec == 8 || comp.prec == 16, quiet);
  }
  ...
  for (uint32_t i = 0; i < opj_image->numcomps; ++i) {
    for (uint32_t y = 0; y < pixels.height(); ++y) {
      const OPJ_INT32* const opj_row =
          opj_image->comps[i].data + y * pixels.width();
      if (WP2Formatbpc(pixels.format()) == 8) {
        uint8_t* const row = pixels.GetRow8(y);
        for (uint32_t x = 0; x < pixels.width(); ++x) {
          row[opj_image->numcomps * x + i] = static_cast<uint8_t>(opj_row[x]);
        }
      } else {
        uint16_t* const row = pixels.GetRow16(y);
        for (uint32_t x = 0; x < pixels.width(); ++x) {
          row[opj_image->numcomps * x + i] = static_cast<uint16_t>(opj_row[x]);
        }
      }
    }
  }
