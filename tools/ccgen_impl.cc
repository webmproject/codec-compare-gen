...
int Main(int argc, const char* const argv[]) {
  ...
  for (; arg_index < argc; ++arg_index) {
    ...
    if (arg == "-h" || arg == "--help") {
      ...
      } else if (codec == "jpeg2000" || codec == "j2k" || codec == "openjpeg") {
        codec_settings.push_back({Codec::kJ2k, subsampling.value});
  ...
  if (lossy) {
    std::vector<std::vector<int>> qualities(static_cast<int>(Codec::kJ2k) + 1);
